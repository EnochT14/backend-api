...
router.post('/candle-data', verifyFirebaseToken, async (req, res) => {
  const { symbol, from, to, resolution, currency } = req.body;
  const cacheKey = `${symbol}-${from}-${to}-${resolution}-${currency}`;
  ...
  try {
    ...
    const response = await fetch('https://api.bavest.co/v0/stock/candle', options);
